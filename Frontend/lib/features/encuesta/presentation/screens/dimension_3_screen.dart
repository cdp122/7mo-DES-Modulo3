...
class Dimension3Screen extends StatefulWidget {
  final String cedulaDocente;
  final Map<String, int> respuestasAcumuladas;

  const Dimension3Screen({
    super.key, 
  ...
  @override
  State<Dimension3Screen> createState() => _Dimension3ScreenState();
}

class _Dimension3ScreenState extends State<Dimension3Screen> with SingleTickerProviderStateMixin {
  ...
  final Color colorTema = const Color(0xFFFF9800); 
  ...
  void initState() {
    ...
    _floatingAnimation = Tween<double>(begin: 0.0, end: 15.0).animate(CurvedAnimation(parent: _animationController, curve: Curves.easeInOut));
  }
  ...
  void _continuar(DimensionEntity dimensionActual) {
    ...
    if (!_respuestas.containsKey(codigoReal)) {
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(content: const Text('Por favor, selecciona una respuesta antes de continuar'), backgroundColor: colorTema),
      );
      return;
    }
    ...
    setState(() {
      ...
      if (_preguntaActualIndex < dimensionActual.reactivos.length - 1) {
        ...
        context.go('/encuesta-resultados', extra: {'cedula': widget.cedulaDocente, 'respuestas': _respuestas});
      }
    });
  }
  ...
  String _obtenerImagenOsoPregunta(int index) => 'assets/images/Oso${[1, 2, 4][index % 3]}.png';
    ...
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        actions: [
          Padding(
            ...
            child: IconButton(
              icon: Icon(AudioService().isMuted ? Icons.volume_off_rounded : Icons.volume_up_rounded, color: colorTema, size: 32),
      ...
      body: BlocBuilder<PreguntasCubit, PreguntasState>(
        builder: (context, state) {
          if (state is PreguntasLoading || state is PreguntasInitial) return Center(child: CircularProgressIndicator(color: colorTema));

          if (state is PreguntasLoaded) {
            final dimensionBD = state.dimensiones.firstWhere((d) => d.orden == 3, orElse: () => state.dimensiones.first);
            if (dimensionBD.reactivos.isEmpty) return const Center(child: Text('No hay preguntas para esta área.'));
            ...
            final imagenActual = _mostrarPista ? 'assets/images/Oso3.png' : _obtenerImagenOsoPregunta(_preguntaActualIndex);
            final bool esUltimaPregunta = _preguntaActualIndex == dimensionBD.reactivos.length - 1;
                    ...
                    child: Padding(
                      ...
                      child: Container(
                        ...
                        decoration: BoxDecoration(
                          ...
                          boxShadow: [BoxShadow(color: colorTema.withOpacity(0.04), blurRadius: 25, offset: const Offset(0, 10))],
                        ),
                        child: Column(
                                ...
                                Row(
                                  ...
                                  children: [
                                    TextButton.icon(
                                      ...
                                      label: const Text('Regresar', style: TextStyle(fontSize: 16, fontWeight: FontWeight.bold)),
                                      style: TextButton.styleFrom(foregroundColor: AppColors.textSecondaryLight),
                                    ),
                                    ElevatedButton(
                                      onPressed: () => _continuar(dimensionBD),
                                      style: ElevatedButton.styleFrom(
                                        backgroundColor: esUltimaPregunta ? const Color(0xFF4CAF50) : colorTema,
                                      ...
                                      child: Row(
                                        ...
                                        children: [
                                          Text(esUltimaPregunta ? 'Finalizar' : 'Siguiente', style: const TextStyle(fontSize: 16, fontWeight: FontWeight.bold)),
                                          if (esUltimaPregunta) const SizedBox(width: 8),
                                          if (esUltimaPregunta) const Icon(Icons.check_circle_outline, size: 20),
                                        ],
                                      ),
                                    ),
                                  ],
